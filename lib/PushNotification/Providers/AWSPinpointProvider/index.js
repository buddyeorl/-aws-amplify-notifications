...
var PlatformNotSupportedError_1 = tslib_1.__importDefault(
	require('../../PlatformNotSupportedError')
);
	...
	function AWSPinpointProvider() {
		...
		_this.configure = function (config) {
			...
			_this.config = tslib_1.__assign(
				tslib_1.__assign({}, _super.prototype.configure.call(_this, config)),
				{ endpointInfo: { channelType: _this.getChannelType() } }
			);
			// some configuration steps should not be re-run even if provider is re-configured for some reason
			if (!_this.configured) {
				// wire up default Pinpoint message event handling
				common_1.addEventListener(
					0 /* BACKGROUND_MESSAGE_RECEIVED */,
					function (message) {
						return _this.recordMessageEvent(
							message,
							types_1.AWSPinpointMessageEvent.BACKGROUND_MESSAGE_RECEIVED
						);
					}
				);
				common_1.addEventListener(
					...
					function (message) {
						return _this.recordMessageEvent(
							message,
							types_1.AWSPinpointMessageEvent.FOREGROUND_MESSAGE_RECEIVED
						);
					}
				);
				var launchNotificationOpenedListener_1 = common_1.addEventListener(
					2 /* LAUNCH_NOTIFICATION_OPENED */,
					function (message) {
						_this.recordMessageEvent(
							message,
							types_1.AWSPinpointMessageEvent.NOTIFICATION_OPENED
						);
						// once we are done with it we can remove the listener
						launchNotificationOpenedListener_1 === null ||
						launchNotificationOpenedListener_1 === void 0
							? void 0
							: launchNotificationOpenedListener_1.remove();
					}
				);
				common_1.addEventListener(
			...
			utils_1.dispatchPushNotificationEvent(
				'pinpointProvider_configured',
				null
			);
			return _this.config;
		};
		_this.registerDevice = function (address) {
			return tslib_1.__awaiter(_this, void 0, void 0, function () {
				...
				return tslib_1.__generator(this, function (_a) {
					switch (_a.label) {
						...
						case 2:
							_a.trys.push([2, 4, , 5]);
							this.config.endpointInfo = tslib_1.__assign(
								tslib_1.__assign({}, this.config.endpointInfo),
								{ address: address }
							);
				...
				return tslib_1.__generator(this, function (_a) {
					switch (_a.label) {
						case 0:
							...
							if (!analyticsEvent) {
								utils_1.logger.debug(
									'A notification missing event information was not recorded'
								);
								return [2 /*return*/];
							}
							if (!!this.initialized) return [3 /*break*/, 2];
						...
						case 1:
							...
							_a.label = 2;
						case 2:
							return [2 /*return*/, this.recordAnalyticsEvent(analyticsEvent)];
					}
				});
			});
		};
