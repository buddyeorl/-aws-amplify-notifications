...
export const extractContent = ({
	...
	platform='Web'
}: PinpointInAppMessage & { platform?: 'Android' | 'IOS' | 'Web' }): InAppMessageContent[] => {
	return (
		message?.Content?.map(content => {
			...
			const getButtonConfig = (button: any, defaultConfig: any) => {
				return platform && button && button[platform]
					? { ...defaultConfig, ...button[platform] }
					: defaultConfig;
			};

			const defaultPrimaryButton= getButtonConfig(PrimaryBtn, PrimaryBtn?.DefaultConfig);
			const defaultSecondaryButton = getButtonConfig(SecondaryBtn, SecondaryBtn?.DefaultConfig);
